...
- **i18n**: flat `T[lang][key]` object in `src/lib/i18n.ts`. `t(lang, key)` helper. Lang stored in RLC store, not in a separate context. All user-visible strings — including the footer — must have keys in both `es` and `en`.
