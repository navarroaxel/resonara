  ...
  switch (action.type) {
    ...
    case "SET_RESISTOR_SYMBOL":
      if (typeof window !== "undefined")
        localStorage.setItem("resistorSymbol", action.resistorSymbol);
      return { ...state, resistorSymbol: action.resistorSymbol };
    case "SET_UNIT_NOTATION":
      if (typeof window !== "undefined")
        localStorage.setItem("unitNotation", action.unitNotation);
...
export function UIProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (savedSymbol === "eu" || savedSymbol === "usa")
      dispatch({ type: "SET_RESISTOR_SYMBOL", resistorSymbol: savedSymbol });

    const savedNotation = localStorage.getItem("unitNotation");
    if (savedNotation === "raw" || savedNotation === "si")
      dispatch({ type: "SET_UNIT_NOTATION", unitNotation: savedNotation });
