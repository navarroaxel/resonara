...
function recomputePoly(
  ...
  return polyMode
    ? calcPolyResult(circuitType, params, harmonics, flags)
    : null;
}
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    case "SET_PARAM": {
      ...
      const polyResults = recomputePoly(
        state.polyMode,
        state.circuitType,
        params,
        state.harmonics,
    ...
    case "TOGGLE_POLY_MODE": {
      ...
      const activeTab =
        !polyMode && state.activeTab === "spectrum"
          ? ("phasor" as ActiveTab)
          : state.activeTab;
    ...
    case "SET_HARMONIC": {
      const harmonics = state.harmonics.map((h, i) =>
        i === action.index ? { ...h, ...action.harmonic } : h,
      );
      const polyResults = recomputePoly(
        state.polyMode,
        state.circuitType,
...
const RLCContext = createContext<{
  state: State;
