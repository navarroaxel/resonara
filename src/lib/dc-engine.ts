...
export function calcDC(params: DCParams): DCResult {
  ...
  // 3×3 determinant by cofactor expansion on row 1 (a13=0)
  const M11 = a22 * a33 - a23 * a32   // cofactor minor for a11
  const M12 = a21 * a33               // cofactor minor for a12 (a31=0)
  const D   = a11 * M11 - a12 * M12

  // Cramer numerators — replace column k with [V1, V3, V2]
  // N1: replace col 1
  ...
  const IR1 = I1
  const IR2 = I1 - I2   // shared Mesh 1–2
  const IR3 = I2
  const IR4 = I2 - I3   // shared Mesh 2–3
  const IR5 = I3
  ...
  // Residuals from direct substitution — always ≈ 0
  const kvl1 = V1 - a11 * I1 - a12 * I2
  const kvl2 = V3 - a21 * I1 - a22 * I2 - a23 * I3
  const kvl3 = V2 - a32 * I2 - a33 * I3

  const kclA = IR1 - IR2 - IR3   // algebraically 0
  const kclB = IR3 - IR4 - IR5   // algebraically 0
