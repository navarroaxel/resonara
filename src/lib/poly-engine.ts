...
function combineByN(harmonics: HarmonicInput[]): HarmonicInput[] {
  ...
  for (const { n, An, phin } of harmonics) {
    ...
    groups.set(n, {
      x: prev.x + An * Math.cos(rad),
      y: prev.y + An * Math.sin(rad),
    });
  }
...
export const PRESETS: Record<Exclude<PolyPreset, "custom">, HarmonicInput[]> = {
  ...
  triangle: [1, 3, 5, 7].map((n, i) => ({
    n,
    An: 1 / (n * n),
    phin: i % 2 === 0 ? 0 : 180,
  })),
  sawtooth: [1, 2, 3, 4, 5, 6].map((n) => ({
    n,
    An: 1 / n,
    phin: n % 2 === 0 ? 180 : 0,
  })),
...
export function calcPolyResult(
  ...
  const harmonicResults: HarmonicResult[] = combineByN(harmonics).map(
    ({ n, An, phin }) => {
      const fn = n * params.f;
  ...
  const I_rms = Math.sqrt(
    harmonicResults.reduce((acc, h) => acc + h.In ** 2 / 2, 0),
  );
  const V_rms = Math.sqrt(
    harmonicResults.reduce((acc, h) => acc + h.Vn ** 2 / 2, 0),
  );
  const THD_I = I1_rms > 0 ? (Math.sqrt(higherSumSq) / I1_rms) * 100 : 0;
  const P_total = harmonicResults.reduce(
    (acc, h) =>
      acc + ((h.Vn * h.In) / 2) * Math.cos((h.phin_circuit * Math.PI) / 180),
    0,
  );
