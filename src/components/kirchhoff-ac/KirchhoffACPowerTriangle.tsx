...
export function KirchhoffACPowerTriangle() {
  ...
  useEffect(() => {
    ...
    const scale =
      Math.min((W - 2 * pad) / maxDim, (H - 2 * pad) / maxDim) * 0.85;
    ...
    const qColor =
      Q > 0 ? (isDark ? "#fb923c" : "#ea580c") : isDark ? "#60a5fa" : "#2563eb";
    ...
    if (Math.abs(phi) > 0.01) {
      ...
      ctx.fillText(
        "φ",
        orig.x + (arcR + 10) * Math.cos(midAng),
        orig.y + (arcR + 10) * Math.sin(midAng),
      );
    }
