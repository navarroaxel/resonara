...
function capacitorV(
  ...
  ctx.lineCap = "round";
  ctx.beginPath();
  ctx.moveTo(cx - plateW / 2, cy - gap / 2);
  ctx.lineTo(cx + plateW / 2, cy - gap / 2);
  ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(cx - plateW / 2, cy + gap / 2);
  ctx.lineTo(cx + plateW / 2, cy + gap / 2);
...
export function KirchhoffACSchematic() {
  ...
  useEffect(() => {
    ...
    ctx.fillText("GND", NODE_A, BOT + 28);
    ctx.restore();
