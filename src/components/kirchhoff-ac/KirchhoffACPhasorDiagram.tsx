...
export function KirchhoffACPhasorDiagram() {
  ...
  useEffect(() => {
    ...
    const ux = (mag: number, angDeg: number) => mag * Math.cos(angDeg * Math.PI / 180)
    const uy = (mag: number, angDeg: number) => -mag * Math.sin(angDeg * Math.PI / 180)  // y flipped

    const p0 = { x: 0, y: 0 }
    const p1u = { x: p0.x + ux(I1.mag, I1.ang), y: p0.y + uy(I1.mag, I1.ang) }
    const p2u = { x: p1u.x + ux(I2.mag, I2.ang), y: p1u.y + uy(I2.mag, I2.ang) }
    const p3u = mesh3
    ...
    const maxExtent = Math.max(...chainPoints.flatMap(p => [Math.abs(p.x), Math.abs(p.y)]), 1e-9)
    const iScale = (SIZE * 0.38) / maxExtent

    // Place current chain origin at canvas center
    const O = { x: cx, y: cy }
    const toC = (p: { x: number; y: number }) => ({ x: O.x + p.x * iScale, y: O.y + p.y * iScale })
    ...
    if (mesh3 && I3.mag > 1e-9) drawArrow(ctx, T2.x, T2.y, T3.x, T3.y, c3, true)
    ...
    if (I1.mag > 1e-9) rows.push({ color: c1, label: `I₁ = ${I1.mag.toFixed(3)} A ∠${I1.ang.toFixed(1)}°` })
    if (I2.mag > 1e-9) rows.push({ color: c2, label: `I₂ = ${I2.mag.toFixed(3)} A ∠${I2.ang.toFixed(1)}°` })
    if (mesh3 && I3.mag > 1e-9) rows.push({ color: c3, label: `I₃ = ${I3.mag.toFixed(3)} A ∠${I3.ang.toFixed(1)}°` })
    if (Imag > 1e-9) rows.push({ color: cR, label: `I  = ${Imag.toFixed(3)} A ∠${Iang.toFixed(1)}°` })
