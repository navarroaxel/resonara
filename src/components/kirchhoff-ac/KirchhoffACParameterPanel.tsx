  ...
  return (
    <div>
      ...
      <div className="space-y-5">
        ...
        {/* PFC — Mesh 3 */}
        <div className="border-neutral-200 pt-2 dark:border-neutral-700">
          <div className="mb-4 flex items-center gap-2 border-b border-neutral-200 pb-1 dark:border-neutral-700">
            <span className="text-xs font-medium tracking-wider text-neutral-400 uppercase">
          ...
          <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 dark:border-amber-800 dark:bg-amber-950/30">
            <p className="mb-0.5 text-xs text-amber-700 dark:text-amber-400">
              {t(lang, "kacCReq")}
            </p>
            <div className="flex items-center justify-between gap-2">
              <p className="text-lg font-semibold text-amber-800 tabular-nums dark:text-amber-300">
                {fmt(results.C_req, 2)}
                <span className="ml-1 text-xs font-normal">µF</span>
              </p>
              <button
                ...
                onClick={() => {
                  const clamped = Math.min(
                    5000,
                    Math.max(0.1, Math.round(results.C_req * 100) / 100),
                  );
                  dispatch({ type: "SET_PARAM", key: "C", value: clamped });
                  if (!flags.mesh3)
                    dispatch({ type: "SET_FLAG", key: "mesh3", value: true });
                }}
                className="rounded bg-amber-500 px-2 py-1 text-xs font-medium text-white transition-colors hover:bg-amber-600 disabled:cursor-not-allowed disabled:opacity-40"
              >
                {t(lang, "kacApplyCReq")}
              </button>
            </div>
            <p className="mt-0.5 text-xs text-amber-600 dark:text-amber-500">
              ...
              {Number.isFinite(results.fp_0) && (
                <span className="ml-2 tabular-nums">
                  · {t(lang, "kacFp0")}: <strong>{fmt(results.fp_0, 4)}</strong>
                </span>
              )}
            </p>
          </div>
