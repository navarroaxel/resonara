...
export function KirchhoffACPFCCurve() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < N; i++) {
      ...
      const res = calcKirchhoffAC(
        { Vs, f, R1, R, Rm, Lm: lm, C: 50 },
        { mesh3: false },
      );
      lms.push(lm);
    ...
    const curIdx = Math.max(
      ...
      Math.min(
        N - 1,
        Math.round(((curLm - LM_MIN) / (LM_MAX - LM_MIN)) * (N - 1)),
      ),
    );
    ctx.fillStyle = amberLine;
    ctx.beginPath();
    ctx.arc(xCur, tyC(cReqs[curIdx]), 4, 0, 2 * Math.PI);
    ctx.fill();
    ctx.fillStyle = violetLine;
    ctx.beginPath();
    ctx.arc(xCur, tyFp(fp0s[curIdx]), 4, 0, 2 * Math.PI);
    ctx.fill();
