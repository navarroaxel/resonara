...
export function ResonanceBadge() {
  const {
    state: { params, results, circuitType, flags },
  } = useRLC();
  const {
    state: { lang },
  } = useUI();
  ...
  if (!flags.hasL && !flags.hasC) {
    ...
    text = t(
      lang,
      circuitType === "series" ? "inductive_series" : "inductive_parallel",
    );
    style =
    ...
    text = t(
      lang,
      circuitType === "series" ? "capacitive_series" : "capacitive_parallel",
    );
    style =
