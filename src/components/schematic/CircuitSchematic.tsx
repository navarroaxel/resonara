...
function drawSeries(
  ...
  const wC = isDark ? "#85B7EB" : "#378ADD",
    nC = isDark ? "#FAC775" : "#BA7517";
  const mC = isDark ? "#9FA0A0" : "#888",
    rC = isDark ? "#F0997B" : "#D85A30";
  ...
  wire(ctx, srcX, bot, srcX, srcY + 22, wC);
  wire(ctx, srcX, top, srcX, srcY - 22, wC);
  sourceAC(ctx, srcX, srcY, 22, mC);
  const rw = 72,
    lw = 80,
    cw = 34;
  const spacing = (right - left - 80 - rw - lw - cw) / 2;
  const rStart = left + 80,
    lStart = rStart + rw + spacing,
    cStart = lStart + lw + spacing;
  ...
  dot(ctx, rStart + rw, top, nC);
  dot(ctx, lStart + lw, top, nC);
  const frStr = Number.isFinite(fr) ? `fr = ${fmt(fr, 1)} Hz` : "fr = —";
  ctx.fillStyle = mC;
  ctx.font = "11px sans-serif";
  ctx.textAlign = "center";
  ctx.fillText(
    `XL = ${fmt(XL, 1)} Ω   XC = ${fmt(XC, 1)} Ω   ${frStr}`,
    W / 2,
    bot + 24,
  );
}
...
function drawParallel(
  ...
  const srcX = left + 22,
    srcY = (top + bot) / 2;
  wire(ctx, srcX, top, right, top, wC);
  wire(ctx, srcX, bot, right, bot, wC);
  wire(ctx, srcX, top, srcX, srcY - 22, wC);
  wire(ctx, srcX, bot, srcX, srcY + 22, wC);
  sourceAC(ctx, srcX, srcY, 22, mC);
  const branches = [
    { x: 210, type: "R", color: isDark ? "#F0997B" : "#D85A30" },
    {
      x: 330,
      type: "L",
      color: hasL ? (isDark ? "#AFA9EC" : "#7F77DD") : grayC,
    },
    {
      x: 440,
      type: "C",
      color: hasC ? (isDark ? "#5DCAA5" : "#1D9E75") : grayC,
    },
  ];
  branches.forEach(({ x, type, color }) => {
    const bx = x + 17;
    wire(ctx, bx, top, bx, top + 14, wC);
    wire(ctx, bx, bot, bx, bot - 14, wC);
    dot(ctx, bx, top, nC);
    dot(ctx, bx, bot, nC);
    if (type === "R") {
      ...
      ctx.lineCap = "round";
      ctx.beginPath();
      ctx.moveTo(bx - 12, mid - 8);
      ctx.lineTo(bx + 12, mid - 8);
      ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(bx - 12, mid + 8);
      ctx.lineTo(bx + 12, mid + 8);
...
export function CircuitSchematic() {
  ...
  const {
    state: { circuitType, results, flags },
  } = useRLC();
  const {
  ...
  return (
    <canvas
      ...
      aria-label={t(
        lang,
        circuitType === "series"
          ? "schematicAriaSeriesLabel"
          : "schematicAriaParallelLabel",
      )}
    />
  );
}
