...
export function BodeChart() {
  ...
  const {
    state: { params, circuitType, results, flags, polyMode, polyResults },
  } = useRLC();
  const {
    state: { lang },
  } = useUI();
  const data = useMemo(
    () => calcBodeCurve(circuitType, params, flags),
    [circuitType, params, flags],
  );
  ...
  useEffect(() => {
    ...
    const toY = (z: number) =>
      pad.top +
      (1 - (z - zMin) / (zMax - zMin || 1)) * (H - pad.top - pad.bottom);
    ...
    ctx.fillText(t(lang, "impedAxisLabel"), 0, 0);
    ctx.restore();
    ...
    ctx.beginPath();
    data.forEach((p, i) =>
      i === 0 ? ctx.moveTo(toX(p.f), toY(p.Z)) : ctx.lineTo(toX(p.f), toY(p.Z)),
    );
    ctx.stroke();
