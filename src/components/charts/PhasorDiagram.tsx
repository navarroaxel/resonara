...
export function PhasorDiagram() {
  ...
  const {
    state: { params, results, circuitType, flags, polyMode, polyResults },
  } = useRLC();
  const {
    state: { lang },
  } = useUI();
  ...
  useEffect(() => {
    ...
    if (polyMode && polyResults && polyResults.harmonics.length > 0) {
      ...
      if (circuitType === "series") {
        ...
        if (flags.hasL)
          drawArrow(ctx, cx, cy, cx, cy - I * XL * scale, "#7F77DD", "VL");
        if (flags.hasC)
          drawArrow(ctx, cx, cy, cx, cy + I * XC * scale, "#C0392B", "VC");
      }
