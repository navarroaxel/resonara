...
export function TimeDomainChart() {
  ...
  const {
    state: { params, circuitType, flags, polyMode, harmonics },
  } = useRLC();
  const {
    state: { lang },
  } = useUI();
  ...
  useEffect(() => {
    ...
    const toYv = (v: number) =>
      pad.top + (1 - (v + vMax) / (2 * vMax)) * (H - pad.top - pad.bottom);
    const toYi = (i: number) =>
      pad.top + (1 - (i + iMax) / (2 * iMax)) * (H - pad.top - pad.bottom);
    ...
    data.forEach((p, i) =>
      i === 0
        ? ctx.moveTo(toX(p.t), toYv(p.v))
        : ctx.lineTo(toX(p.t), toYv(p.v)),
    );
    ctx.stroke();
    ...
    data.forEach((p, i) =>
      i === 0
        ? ctx.moveTo(toX(p.t), toYi(p.i))
        : ctx.lineTo(toX(p.t), toYi(p.i)),
    );
    ctx.stroke();
    ctx.setLineDash([]);
    ...
    ctx.textAlign = "left";
    ctx.fillStyle = "#378ADD";
    ctx.fillRect(pad.left, pad.top - 14, 20, 3);
    ctx.fillText("u(t)", pad.left + 24, pad.top - 8);
    ctx.strokeStyle = "#1D9E75";
    ctx.lineWidth = 2;
    ctx.setLineDash([5, 3]);
    ctx.beginPath();
    ctx.moveTo(pad.left + 62, pad.top - 12);
    ctx.lineTo(pad.left + 82, pad.top - 12);
    ctx.stroke();
    ctx.setLineDash([]);
    ctx.fillStyle = "#1D9E75";
    ctx.fillText("i(t)", pad.left + 86, pad.top - 8);
  }, [data, lang]);
