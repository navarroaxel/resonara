...
export function MagneticFreqResponse() {
  ...
  useEffect(() => {
    ...
    function toY(i: number) {
      return PAD.top + plotH - (i / iMax) * plotH;
    }
    ...
    ctx.beginPath();
    ctx.arc(opX, toY(results.I1), 4, 0, Math.PI * 2);
    ctx.fill();
    if (results.I2 > 1e-9) {
      ctx.fillStyle = i2Color;
      ctx.beginPath();
      ctx.arc(opX, toY(results.I2), 4, 0, Math.PI * 2);
      ctx.fill();
    }
    ...
    ctx.setLineDash([3, 3]);
    ctx.beginPath();
    ctx.moveTo(opX, PAD.top);
    ctx.lineTo(opX, PAD.top + plotH);
    ctx.stroke();
    ctx.setLineDash([]);
