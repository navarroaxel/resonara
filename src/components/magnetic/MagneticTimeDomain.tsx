...
export function MagneticTimeDomain() {
  ...
  useEffect(() => {
    ...
    function toX(s: number) {
      return PAD.left + (s / SAMPLES) * plotW;
    }
    function toYv(v: number) {
      return PAD.top + plotH / 2 - (v / vPeak) * (plotH / 2 - 4);
    }
    function toYi(i: number) {
      return PAD.top + plotH / 2 - (i / iPeak) * (plotH / 2 - 4);
    }
