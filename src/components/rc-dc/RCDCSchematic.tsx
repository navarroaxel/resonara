...
function capacitorV(ctx: CanvasRenderingContext2D, cx: number, cy: number, hh: number, c: string, mC: string, lbl: string) {
  const gapY = 8, plateW = 28
  ...
  ctx.beginPath(); ctx.moveTo(cx - plateW / 2, cy - gapY); ctx.lineTo(cx + plateW / 2, cy - gapY); ctx.stroke()
  ctx.beginPath(); ctx.moveTo(cx - plateW / 2, cy + gapY); ctx.lineTo(cx + plateW / 2, cy + gapY); ctx.stroke()
  ctx.restore()
  ctx.save(); ctx.fillStyle = c; ctx.font = 'bold 12px sans-serif'; ctx.textAlign = 'left'; ctx.textBaseline = 'middle'
...
function sourceDC(ctx: CanvasRenderingContext2D, cx: number, cy: number, r: number, c: string, mC: string, voltage: number) {
  ctx.save(); ctx.strokeStyle = c; ctx.lineWidth = 1.8
  ctx.beginPath(); ctx.arc(cx, cy, r, 0, 2 * Math.PI); ctx.stroke()
  ctx.fillStyle = c; ctx.font = '12px sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'
  ctx.fillText('+', cx, cy - 9)
  ctx.fillText('−', cx, cy + 10)
  ctx.font = 'bold 12px sans-serif'; ctx.textAlign = 'right'; ctx.textBaseline = 'alphabetic'
...
export function RCDCSchematic() {
  ...
  useEffect(() => {
    ...
    const LEFT = 80, RIGHT = 480, TOP = 40, BOT = 180
    ...
    if (Number.isFinite(I) && I > 0) {
      ...
      ctx.save(); ctx.fillStyle = iC; ctx.font = '10px sans-serif'
    ...
    ctx.save(); ctx.fillStyle = cC; ctx.font = '11px sans-serif'
    ...
    const tauStr = tau < 1 ? `τ = ${fmt(tau * 1000, 1)} ms` : `τ = ${fmt(tau, 3)} s`
    ctx.save(); ctx.fillStyle = mC; ctx.font = '11px sans-serif'
