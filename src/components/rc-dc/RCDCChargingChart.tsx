...
function timeUnit(tau: number) {
  return tau < 1 ? "ms" : "s";
}
function timeScale(tau: number) {
  return tau < 1 ? 1000 : 1;
}
...
export function RCDCChargingChart() {
  ...
  useEffect(() => {
    ...
    function xOf(t: number) {
      return PAD.left + (t / tMax) * cW;
    }
    function yVOf(v: number) {
      return PAD.top + cH - (v / Vs) * cH;
    }
    function yIOf(i: number) {
      return PAD.top + cH - (i / I0) * cH;
    }
    ...
    ctx.setLineDash([2, 2]);
    ctx.beginPath();
    ctx.moveTo(PAD.left, y63);
    ctx.lineTo(x63, y63);
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(x63, PAD.top + cH);
    ctx.lineTo(x63, y63);
    ctx.stroke();
    ctx.setLineDash([]);
    ctx.beginPath();
    ctx.arc(x63, y63, 4, 0, 2 * Math.PI);
    ctx.fill();
    ctx.font = "10px sans-serif";
    ctx.textAlign = "left";
    ctx.textBaseline = "bottom";
